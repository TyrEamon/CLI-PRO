...
func (s *Store) init() error {
	...
	return s.ensureModelPriceColumns()
}

func (s *Store) ensureModelPriceColumns() error {
	columns, err := s.tableColumns("model_prices")
	if err != nil {
		return err
	}
	for _, column := range []struct {
		...
		{"source", "source text"},
		{"source_model_id", "source_model_id text"},
		{"raw_json", "raw_json text"},
		{"synced_at_ms", "synced_at_ms integer not null default 0"},
	} {
		if columns[column.name] {
			continue
		}
		if _, err := s.db.Exec("alter table model_prices add column " + column.definition); err != nil {
			return err
		}
	}
	return nil
...
func (s *Store) DeadLetters(ctx context.Context, limit int, offset int) (DeadLetterPage, error) {
	if limit <= 0 {
		limit = 50
	}
	if limit > 500 {
	...
	page := DeadLetterPage{Limit: limit, Offset: offset}
	if err := s.db.QueryRowContext(ctx, `select count(*) from dead_letter_events`).Scan(&page.Total); err != nil {
		return page, err
	}
	rows, err := s.db.QueryContext(ctx, `select id, payload, error, created_at_ms from dead_letter_events order by id desc limit ? offset ?`, limit, offset)
	if err != nil {
		return page, err
	}
	defer rows.Close()

	page.Items = make([]DeadLetterEntry, 0)
	for rows.Next() {
		var entry DeadLetterEntry
		if err := rows.Scan(&entry.ID, &entry.Payload, &entry.Error, &entry.CreatedAtMS); err != nil {
			return page, err
		}
...
func (s *Store) WriteDeadLettersJSONL(ctx context.Context, writer io.Writer) error {
	rows, err := s.db.QueryContext(ctx, `select id, payload, error, created_at_ms from dead_letter_events order by id asc`)
	if err != nil {
		return err
	}
	defer rows.Close()
...
func (s *Store) ExportJSONL(ctx context.Context) ([]byte, error) {
	...
	output := make([]byte, 0)
	if len(prices) > 0 {
		output, err = appendJSONLine(output, modelPricesExportRecord{
	...
	for i := len(events) - 1; i >= 0; i-- {
		event := events[i]
		event.RawJSON = ""
		output, err = appendJSONLine(output, event)
		if err != nil {
			return nil, err
		}
	}
...
func appendJSONLine(output []byte, value any) ([]byte, error) {
...
func (s *Store) GetModelPrices(ctx context.Context) (map[string]ModelPrice, error) {
	rows, err := s.db.QueryContext(ctx, `select model, prompt_price, completion_price, cache_price, coalesce(source, ''), coalesce(source_model_id, ''), coalesce(raw_json, ''), updated_at_ms, coalesce(synced_at_ms, 0) from model_prices order by model asc`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var rawJSON string
		if err := rows.Scan(&model, &price.Prompt, &price.Completion, &price.Cache, &price.Source, &price.SourceModelID, &rawJSON, &price.UpdatedAtMS, &price.SyncedAtMS); err != nil {
			return nil, err
		}
		if rawJSON != "" {
			price.RawJSON = json.RawMessage(rawJSON)
		}
		prices[model] = price
	}
	if err := rows.Err(); err != nil {
...
func (s *Store) SetModelPrices(ctx context.Context, prices map[string]ModelPrice) error {
	...
	if err := insertModelPrices(ctx, tx, prices, 0); err != nil {
		return err
	}
	return tx.Commit()
}

func (s *Store) UpsertModelPrices(ctx context.Context, prices map[string]ModelPrice, syncedAt int64) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if err := insertModelPrices(ctx, tx, prices, syncedAt); err != nil {
		return err
	}
	return tx.Commit()
}

func insertModelPrices(ctx context.Context, tx *sql.Tx, prices map[string]ModelPrice, syncedAt int64) error {
	now := time.Now().UnixMilli()
	stmt, err := tx.PrepareContext(ctx, `insert into model_prices(model, prompt_price, completion_price, cache_price, source, source_model_id, raw_json, updated_at_ms, synced_at_ms)
		values(?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		where model_prices.prompt_price is not excluded.prompt_price
			or model_prices.completion_price is not excluded.completion_price
			or model_prices.cache_price is not excluded.cache_price
			or model_prices.source is not excluded.source
			or model_prices.source_model_id is not excluded.source_model_id
			or model_prices.raw_json is not excluded.raw_json`)
	if err != nil {
		return err
	}
	defer stmt.Close()
	...
	for model, price := range prices {
		...
		updatedAt := price.UpdatedAtMS
		if updatedAt <= 0 {
			updatedAt = now
		}
		priceSyncedAt := price.SyncedAtMS
		if syncedAt > 0 {
			priceSyncedAt = syncedAt
		}
		if _, err := stmt.ExecContext(ctx, model, price.Prompt, price.Completion, price.Cache, nullString(price.Source), nullString(price.SourceModelID), nullString(string(price.RawJSON)), updatedAt, priceSyncedAt); err != nil {
			return err
		}
	}
	return nil
