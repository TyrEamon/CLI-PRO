...
- `GET /v0/management/usage/status` — 服务状态、记录数量和 ingestion runtime stats。
- `GET /v0/management/usage/dead-letters` — 分页查看无法解析的 usage payload dead letters。
- `GET /v0/management/usage/dead-letters/export` — dead-letter NDJSON 导出，与普通 usage 导出分离。
...
- `POST /v0/management/usage/model-prices/sync` — 从 LiteLLM 同步模型价格。

### Usage 状态、dead letters 和模型价格

status 接口会返回已保存 event 数量、dead-letter 数量，以及 ingestion runtime stats，例如最近消费/写入时间、累计 inserted/skipped 和最近一次 ingestion 错误。

无法解析的 usage payload 会保存为 dead letters，用于可观测性。普通 `/usage/export` 会有意排除 dead letters，避免常规备份变大；需要时可通过 `/usage/dead-letters/export` 单独导出。

模型价格保存在 SQLite 中，可手动管理，也可从 LiteLLM 同步。LiteLLM 的 per-token 价格会在入库前归一化为管理端使用的 `/1M tokens` 成本单位。
