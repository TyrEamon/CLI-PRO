...
- `GET /v0/management/usage/status` — service status, record counts, and ingestion runtime stats.
- `GET /v0/management/usage/dead-letters` — paginated dead-letter records for malformed usage payloads.
- `GET /v0/management/usage/dead-letters/export` — dead-letter NDJSON export, separate from normal usage export.
...
- `POST /v0/management/usage/model-prices/sync` — sync model prices from LiteLLM.

### Usage status, dead letters, and model prices

The status endpoint reports stored event counts, dead-letter counts, and runtime ingestion stats such as last consumed/inserted timestamps, inserted/skipped totals, and the last ingestion error.

Malformed usage payloads are stored as dead letters for observability. Normal `/usage/export` intentionally excludes dead letters so regular backups stay compact; export them explicitly with `/usage/dead-letters/export` when needed.

Model prices are stored in SQLite and can be managed manually or synced from LiteLLM. LiteLLM per-token prices are normalized to the UI's `/1M tokens` cost unit before persistence.
