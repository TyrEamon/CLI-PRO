...
const backendProgressStatus = (status: AccountInspectionScheduleResponse['status']): AccountInspectionProgressSnapshot['status'] => {
  ...
  if (status.state === 'stopped' || status.state === 'stopping') return 'stopped';
  if (status.lastFinishedAt > 0) return 'completed';
  return 'idle';
};
...
const backendRunStatus = (status: AccountInspectionScheduleResponse['status']): RunStatus => {
  ...
  if (status.state === 'stopped' || status.state === 'stopping') return 'idle';
  if (status.lastFinishedAt > 0) return status.lastError ? 'error' : 'success';
  return 'idle';
};
...
const applyBackendInspectionResponse = (
  ...
  const completed = backendProgress?.completed ?? (response.status.running ? 0 : total);
  const inFlight = backendProgress?.inFlight ?? (response.status.running ? 1 : 0);
