...
type RealtimeSortKey = 'timestampMs' | 'totalTokens' | 'totalCost' | 'latencyMs' | 'requestCount' | 'successRate';
type RealtimeSortDirection = 'asc' | 'desc';

type RealtimeSortState = {
  key: RealtimeSortKey;
...
const toLocalInputDateTime = (timestampMs: number) => {
  const date = new Date(timestampMs);
  const offsetMs = date.getTimezoneOffset() * 60 * 1000;
  return new Date(date.getTime() - offsetMs).toISOString().slice(0, 16);
};

const parseLocalInputDateTime = (value: string): number | null => {
...
const getRealtimeSortValue = (row: RealtimeLogRow, key: RealtimeSortKey) => {
  switch (key) {
    case 'totalTokens':
      return row.totalTokens;
    case 'totalCost':
...
export function MonitoringCenterPage() {
  ...
  const [customStart, setCustomStart] = useState(() => toLocalInputDateTime(Date.now() - 24 * 60 * 60 * 1000));
  const [customEnd, setCustomEnd] = useState(() => toLocalInputDateTime(Date.now()));
  ...
  const [isSyncingModelPrices, setIsSyncingModelPrices] = useState(false);
  const [usageStatus, setUsageStatus] = useState<UsageStatusPayload | null>(null);
  const [deadLetters, setDeadLetters] = useState<DeadLetterPagePayload | null>(null);
  ...
  const [realtimeSort, setRealtimeSort] = useState<RealtimeSortState>({ key: 'timestampMs', direction: 'desc' });
  ...
  const customTimeRange = useMemo<MonitoringCustomTimeRange>(() => ({
    startMs: parseLocalInputDateTime(customStart),
    endMs: parseLocalInputDateTime(customEnd),
  ...
  const loadUsageObservability = useCallback(async () => {
    if (connectionStatus !== 'connected') return;
    const [status, deadLetterPage] = await Promise.all([
      apiClient.get<UsageStatusPayload>('/usage/status'),
      apiClient.get<DeadLetterPagePayload>('/usage/dead-letters?limit=5'),
    ]);
    setUsageStatus(status ?? null);
    setDeadLetters(deadLetterPage ?? null);
  }, [connectionStatus]);

  const refreshAll = useCallback(async () => {
    await Promise.all([loadUsage(), refreshMeta(false), loadUsageObservability()]);
  }, [loadUsage, loadUsageObservability, refreshMeta]);
  ...
  const handleSyncModelPrices = useCallback(async () => {
    ...
    setIsSyncingModelPrices(true);
    try {
      const result = await syncModelPrices();
      showNotification(
        t('usage_stats.model_price_sync_success', {
          count: result.synced,
          defaultValue: `Synced ${result.synced} model prices from LiteLLM`,
        }),
        'success'
      );
    } catch (error) {
      ...
      setIsSyncingModelPrices(false);
    }
  }, [connectionStatus, showNotification, syncModelPrices, t]);

  const handleExportDeadLetters = useCallback(async () => {
    if (connectionStatus !== 'connected') {
      showNotification(t('notification.connection_required'), 'warning');
      return;
    }
    ...
    try {
      const response = await apiClient.getRaw('/usage/dead-letters/export', { responseType: 'blob' });
      const blob = response.data instanceof Blob ? response.data : new Blob([response.data], { type: 'application/x-ndjson' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      link.href = url;
      link.download = `usage-dead-letters-${timestamp}.jsonl`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
      showNotification(t('usage_stats.dead_letter_export_success', { defaultValue: 'Dead-letter export downloaded' }), 'success');
    } catch (error) {
      showNotification(error instanceof Error ? error.message : String(error || t('common.unknown_error')), 'error');
    }
  }, [connectionStatus, showNotification, t]);
  ...
  useEffect(() => {
    void loadUsageObservability().catch(() => {});
  }, [loadUsageObservability]);
  ...
  const sortedRealtimeLogRows = useMemo(() => {
    const directionFactor = realtimeSort.direction === 'desc' ? -1 : 1;
    return [...realtimeLogRows].sort((left, right) => {
      const valueDiff = getRealtimeSortValue(left, realtimeSort.key) - getRealtimeSortValue(right, realtimeSort.key);
      if (valueDiff !== 0) return valueDiff * directionFactor;
      return right.timestampMs - left.timestampMs || right.id.localeCompare(left.id);
    });
  }, [realtimeLogRows, realtimeSort]);
  const realtimePageSizeNumber = Number(realtimePageSize) || 50;
  const realtimeTotalPages = Math.max(1, Math.ceil(sortedRealtimeLogRows.length / realtimePageSizeNumber));
  const boundedRealtimePage = Math.min(realtimePage, realtimeTotalPages);
  const pagedRealtimeLogRows = useMemo(
    () => sortedRealtimeLogRows.slice((boundedRealtimePage - 1) * realtimePageSizeNumber, boundedRealtimePage * realtimePageSizeNumber),
    [boundedRealtimePage, realtimePageSizeNumber, sortedRealtimeLogRows]
  );
  ...
  useEffect(() => {
    setRealtimePage(1);
  }, [deferredSearch, realtimePageSize, selectedAccount, selectedChannel, selectedModel, selectedProvider, selectedStatus, timeRange, customStart, customEnd]);

  useEffect(() => {
    if (realtimePage > realtimeTotalPages) {
      setRealtimePage(realtimeTotalPages);
    }
  }, [realtimePage, realtimeTotalPages]);

  const failureBreakdownRows = useMemo(() => {
    const map = new Map<string, { label: string; failures: number; total: number; latest: number }>();
    scopedRows.forEach((row) => {
      const label = [row.provider || '-', row.model || '-', row.channel || '-'].join(' · ');
      const current = map.get(label) ?? { label, failures: 0, total: 0, latest: 0 };
      current.total += 1;
      if (row.failed) current.failures += 1;
      current.latest = Math.max(current.latest, row.timestampMs);
      map.set(label, current);
    });
    return Array.from(map.values())
      .filter((row) => row.failures > 0)
      .sort((left, right) => right.failures - left.failures || right.latest - left.latest)
      .slice(0, 8);
  }, [scopedRows]);
  ...
  const handleRealtimeSort = useCallback((key: RealtimeSortKey) => {
    setRealtimeSort((previous) =>
      previous.key === key
  ...
  return (
    <div className={styles.page}>
          ...
          <div className={styles.mastheadControlRow}>
            <div className={styles.segmentedControl}>
              {TIME_RANGE_OPTIONS.map((option) => (
                ...
                >
                  {t(option.labelKey, {
                    defaultValue: option.value === 'custom' ? 'Custom' : option.labelKey,
                  })}
                </button>
              ))}
            </div>
            ...
            {timeRange === 'custom' ? (
              <div className={styles.customRangeControls}>
                <Input
                  ...
                  aria-label={t('monitoring.range_custom_start', { defaultValue: 'Custom range start' })}
                />
                <Input
                  type="datetime-local"
                  value={customEnd}
                  onChange={(event) => setCustomEnd(event.target.value)}
                  aria-label={t('monitoring.range_custom_end', { defaultValue: 'Custom range end' })}
                />
              </div>
            ) : null}
      ...
      <Panel
        title={t('monitoring.failure_breakdown_title', { defaultValue: 'Failure breakdown' })}
        subtitle={t('monitoring.failure_breakdown_desc', { defaultValue: 'Top failed provider/model/channel combinations in the current scope.' })}
      >
        <div className={styles.inlineMetrics}>
          {failureBreakdownRows.length > 0 ? (
            failureBreakdownRows.map((row) => (
              <span key={row.label}>{`${row.label}: ${row.failures}/${row.total} (${formatPercent(row.total > 0 ? row.failures / row.total : 0)})`}</span>
            ))
          ) : (
            <span>{t('monitoring.no_recent_failures', { defaultValue: 'No failures in current scope' })}</span>
          )}
        </div>
      </Panel>

      <Panel
        title={t('usage_stats.observability_title', { defaultValue: 'Usage ingestion status' })}
        subtitle={t('usage_stats.observability_desc', { defaultValue: 'Dead letters are observed separately and are not included in normal usage export.' })}
      >
        <div className={styles.inlineMetrics}>
          <span>{`${t('usage_stats.events', { defaultValue: 'Events' })}: ${usageStatus?.events ?? 0}`}</span>
          <span>{`${t('usage_stats.dead_letters', { defaultValue: 'Dead letters' })}: ${usageStatus?.deadLetters ?? 0}`}</span>
          <span>{`${t('usage_stats.inserted', { defaultValue: 'Inserted' })}: ${usageStatus?.runtime?.totalInserted ?? 0}`}</span>
          <span>{`${t('usage_stats.skipped', { defaultValue: 'Skipped' })}: ${usageStatus?.runtime?.totalSkipped ?? 0}`}</span>
          {usageStatus?.runtime?.lastError ? <span className={styles.badText}>{usageStatus.runtime.lastError}</span> : null}
        </div>
        {deadLetters?.items?.length ? (
          <div className={styles.deadLetterList}>
            {deadLetters.items.map((entry) => (
              <div key={entry.id} className={styles.deadLetterItem}>
                <strong>{new Date(entry.createdAtMs).toLocaleString(i18n.language)}</strong>
      ...
      <Panel
        ...
        extra={
          <div className={styles.inlineMetrics}>
            ...
            <span>{`${t('usage_stats.dead_letters', { defaultValue: 'Dead letters' })}: ${usageStatus?.deadLetters ?? deadLetters?.total ?? 0}`}</span>
            <button type="button" className={styles.inlineActionButton} onClick={() => void handleExportDeadLetters()}>
              {t('usage_stats.export_dead_letters', { defaultValue: 'Export dead letters' })}
            </button>
          </div>
        }
      >
          ...
          <table className={`${styles.table} ${styles.realtimeTable}`}>
            <thead>
              <tr>
                ...
                <th>
                  <button type="button" className={styles.sortableHeaderButton} onClick={() => handleRealtimeSort('successRate')}>
                    {t('monitoring.column_success_rate')}
                  </button>
                </th>
                <th>
                  <button type="button" className={styles.sortableHeaderButton} onClick={() => handleRealtimeSort('requestCount')}>
                    {t('monitoring.total_calls')}
                  </button>
                </th>
                <th>
                  <button type="button" className={styles.sortableHeaderButton} onClick={() => handleRealtimeSort('latencyMs')}>
                    {t('monitoring.column_latency')}
                  </button>
                </th>
                <th>
                  <button type="button" className={styles.sortableHeaderButton} onClick={() => handleRealtimeSort('timestampMs')}>
                    {t('monitoring.column_time')}
                  </button>
                </th>
                <th>
                  <button type="button" className={styles.sortableHeaderButton} onClick={() => handleRealtimeSort('totalTokens')}>
                    {t('monitoring.this_call_usage')}
                  </button>
                </th>
                <th>
                  <button type="button" className={styles.sortableHeaderButton} onClick={() => handleRealtimeSort('totalCost')}>
                    {t('monitoring.this_call_cost')}
                  </button>
                </th>
              </tr>
            </thead>
            <tbody>
              {pagedRealtimeLogRows.map((row) => (
        ...
        <div className={styles.paginationBar}>
          <span>
            {t('monitoring.pagination_summary', {
              page: boundedRealtimePage,
              pages: realtimeTotalPages,
              total: sortedRealtimeLogRows.length,
              defaultValue: `Page ${boundedRealtimePage}/${realtimeTotalPages} · ${sortedRealtimeLogRows.length} rows`,
            })}
          </span>
          <Select
            value={realtimePageSize}
            options={PAGE_SIZE_OPTIONS.map((value) => ({ value, label: `${value} / page` }))}
            onChange={setRealtimePageSize}
            ariaLabel={t('monitoring.page_size', { defaultValue: 'Page size' })}
            fullWidth={false}
          />
          <Button
            variant="secondary"
            ...
            disabled={boundedRealtimePage <= 1}
          >
          ...
          <Button
            variant="secondary"
            size="sm"
            onClick={() => setRealtimePage((page) => Math.min(realtimeTotalPages, page + 1))}
            disabled={boundedRealtimePage >= realtimeTotalPages}
          >
      ...
      >
        <div className={styles.priceEditor}>
          ...
          <div className={styles.priceActionsBar}>
            <Button variant="secondary" size="sm" onClick={handleSyncModelPrices} disabled={isSyncingModelPrices}>
              {isSyncingModelPrices
                ? t('common.loading')
                : t('usage_stats.model_price_sync_litellm', { defaultValue: 'Sync LiteLLM Prices' })}
            </Button>
