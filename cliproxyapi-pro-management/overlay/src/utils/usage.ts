...
const MODEL_PRICE_SYNC_API_PATH = '/usage/model-prices/sync';
...
const normalizeModelPrices = (value: unknown): Record<string, ModelPrice> => {
  ...
  Object.entries(value).forEach(([model, price]) => {
    ...
    const source = typeof price.source === 'string' ? price.source : undefined;
    const sourceModelId = typeof price.sourceModelId === 'string' ? price.sourceModelId : undefined;
    const updatedAtMs = toFiniteNumber(price.updatedAtMs);
    const syncedAtMs = toFiniteNumber(price.syncedAtMs);
...
export async function syncModelPricesFromLiteLLM(): Promise<{ prices: Record<string, ModelPrice>; synced: number; syncedAtMs: number }> {
  const result = await apiClient.post<{ synced?: number; syncedAtMs?: number }>(MODEL_PRICE_SYNC_API_PATH, {});
  const prices = await loadModelPricesFromSqlite();
  return {
    prices,
    synced: Number(result?.synced) || Object.keys(prices).length,
    syncedAtMs: Number(result?.syncedAtMs) || Date.now(),
  };
}
