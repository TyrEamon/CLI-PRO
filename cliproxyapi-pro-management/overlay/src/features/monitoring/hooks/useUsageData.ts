...
export interface UseUsageDataReturn {
  ...
  syncModelPrices: () => Promise<{ synced: number; syncedAtMs: number }>;
...
export function useUsageData(): UseUsageDataReturn {
  ...
  const syncModelPrices = useCallback(async () => {
    const result = await syncModelPricesFromLiteLLM();
    setModelPricesState(result.prices);
    return { synced: result.synced, syncedAtMs: result.syncedAtMs };
  }, []);
