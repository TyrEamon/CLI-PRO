...
export interface UseMonitoringDataParams {
  ...
  customTimeRange?: MonitoringCustomTimeRange;
  searchQuery: string;
}
...
const buildRangeFilteredRows = (
  ...
  timeRange: MonitoringTimeRange,
  customTimeRange: MonitoringCustomTimeRange | undefined,
  searchQuery: string
) => {
  const nowMs = Date.now();
  const startMs = timeRange === 'custom' ? customTimeRange?.startMs ?? Number.NEGATIVE_INFINITY : getRangeStartMs(timeRange, nowMs);
  const endMs = timeRange === 'custom' ? customTimeRange?.endMs ?? nowMs : nowMs;
  const normalizedQuery = searchQuery.trim().toLowerCase();

  return rows.filter((row) => {
    if (row.timestampMs > endMs || row.timestampMs < startMs) {
      return false;
    }
