...
- 基于可配置模型价格的成本估算
- 今日、7 天、14 天、30 天、全部数据和自定义起止时间过滤
- 搜索，以及账号/provider/model/channel/status 过滤
- 自动刷新间隔选择和手动刷新
- 可排序的账号汇总表
- 可展开的账号行，展示模型花费明细
- 当前范围内按 provider/model/channel 聚合的失败分布
- usage ingestion 状态和 dead-letter 摘要面板
- 账号级配额刷新和配额展示
- 可排序、可分页的实时请求表，展示最近成功/失败模式条
...
页面也可以触发后端 LiteLLM 同步。同步后的价格包含来源元数据，并以成本视图使用的 `/1M tokens` 单位持久化到 SQLite。
...
UI 会在读取完整 quota-cache 前先做 stats-only 新鲜度检查。这样账号巡检和配额管理页面能看到后端写入的最新巡检结果，同时避免每次页面切换都批量回读全部配额缓存。
...
- usage ingestion 状态/dead-letter 可观测性和显式 dead-letter 导出。
