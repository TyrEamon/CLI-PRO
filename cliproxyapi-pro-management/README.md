...
- time-range filtering for today, 7d, 14d, 30d, all data, and custom start/end ranges
...
- failure breakdown by provider/model/channel in the current scope
- usage ingestion status and dead-letter summary panel
- account-scoped quota refresh and quota display
- sortable and paginated realtime request table with recent success/failure pattern bars
...
The page can also trigger backend LiteLLM sync. Synced prices include source metadata and are persisted in SQLite with the same `/1M tokens` unit used by the cost view.
...
The UI uses a stats-only freshness check before loading full quota-cache data. This keeps account inspection and quota-management pages current with backend-written inspection results without repeatedly reloading every cached quota entry on navigation.
...
- usage ingestion status/dead-letter observability and explicit dead-letter export.
